...
:
    ...
    mapFieldMax_(1)
{
    const reactionModels::singleStepReaction& ssReaction =
        mesh.lookupObject<reactionModels::singleStepReaction>
        (
            reactionModel::reactionPropertiesName
        );

    const reaction& singleReaction = ssReaction.singleReaction();
    ...
    forAll(singleReaction.rhs(), i)
    {
        ...
        Wm += Xi[i]*ssReaction.thermo().WiValue(speciei);
    }

    scalarList Yprod0(ssReaction.thermo().species().size(), 0.0);

    forAll(singleReaction.rhs(), i)
    {
        const label speciei = singleReaction.rhs()[i].index;
        Yprod0[speciei] = ssReaction.thermo().WiValue(speciei)/Wm*Xi[i];
    }
    ...
    if (mappingFieldName_ == "none")
    {
        ...
        mappingFieldName_ = ssReaction.thermo().Y(index).name();
    }

    const label mapFieldIndex =
        ssReaction.thermo().species()[mappingFieldName_];
