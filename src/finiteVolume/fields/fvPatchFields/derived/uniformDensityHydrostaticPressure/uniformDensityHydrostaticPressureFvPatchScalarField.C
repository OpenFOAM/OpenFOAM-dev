...
:
    ...
    rho_(dict.lookup<scalar>("rhoRef", dimensions::density)),
    pRef_(dict.lookup<scalar>("pRef", dimensions::pressure)),
    pRefPointSpecified_(dict.found("pRefPoint")),
    pRefPoint_
    (
        dict.lookupOrDefault<vector>("pRefPoint", dimensions::length, Zero)
    )
{
    if (dict.found("value"))
    {
