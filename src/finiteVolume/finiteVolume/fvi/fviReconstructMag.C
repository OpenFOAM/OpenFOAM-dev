...
tmp<VolInternalField<scalar>> reconstructMag(const surfaceScalarField& ssf)
{
    ...
    tmp<VolInternalField<scalar>> treconField
    (
...
tmp<VolInternalField<scalar>> reconstructMag
(
    const tmp<surfaceScalarField>& tssf
)
{
    tmp<VolInternalField<scalar>> tvf
    (
        fvi::reconstructMag(tssf())
    );
    tssf.clear();
    return tvf;
}
