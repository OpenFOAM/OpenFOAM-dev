...
template<class Type>
tmp<VolInternalField<scalar>> magSqrGradGrad
(
    const VolField<Type>& vf
)
{
    tmp<VolInternalField<scalar>> tMagSqrGradGrad
    (
        magSqr(fvi::grad(fvc::grad(vf.component(0))))
    );
...
{
    tmp<VolInternalField<scalar>> tMagSqrGradGrad(fvi::magSqrGradGrad(tvf()));
    tvf.clear();
    return tMagSqrGradGrad;
}
